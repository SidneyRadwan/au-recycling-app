...
class CouncilControllerTest {
    ...
    void listCouncils_shouldReturnPageOfCouncils() throws Exception {
        CouncilDto council = new CouncilDto(
                1L, "City of Sydney", "city-of-sydney", "NSW",
                "https://www.cityofsydney.nsw.gov.au", null, "Sydney council"
        );
    ...
    void listCouncils_withStateFilter_shouldReturnFilteredResults() throws Exception {
        CouncilDto council = new CouncilDto(
                1L, "City of Melbourne", "city-of-melbourne", "VIC",
                "https://www.melbourne.vic.gov.au", null, "Melbourne council"
        );
        Page<CouncilDto> page = new PageImpl<>(List.of(council));
        when(councilService.findByState(eq("VIC"), any(Pageable.class))).thenReturn(page);

        mockMvc.perform(get("/api/v1/councils").param("state", "VIC"))
                .andExpect(status().isOk())
    ...
    void getCouncil_shouldReturnCouncilDetail() throws Exception {
        CouncilDetailDto detail = new CouncilDetailDto(
                1L, "City of Sydney", "city-of-sydney", "NSW",
                "https://www.cityofsydney.nsw.gov.au", null, "Sydney council",
                Map.of("RECYCLING", List.of(
                        new CouncilDetailDto.MaterialBinInfo(1L, "Paper", "paper", "Paper & Cardboard", "Place in yellow bin", null)
                ))
        );
        when(councilService.findBySlug("city-of-sydney")).thenReturn(Optional.of(detail));

        mockMvc.perform(get("/api/v1/councils/city-of-sydney"))
                .andExpect(status().isOk())
    ...
    void getCouncil_notFound_shouldReturn404() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/councils/nonexistent"))
